...
using FamilySocialMediaApp.Models;
using FamilySocialMediaApp.Models.Account;
using FamilySocialMediaApp.Security;
using Microsoft.AspNetCore.Mvc;

namespace FamilySocialMediaApp.Controllers
{
    [ApiController]
    public class AccountController : Controller
    {
        ...
        public IActionResult Register(AuthenticationModel model)
        {
            ...
            var user = new User { Password = passwordHash.Password, Salt = passwordHash.Salt, Role = "User", Email = model.Email, FirstName = model.FirstName, LastName = model.LastName };

            // Save the user
            userDao.CreateUser(user);

            // Generate a token
            var token = tokenGenerator.GenerateToken(user.Email, user.FirstName, user.Role);
        ...
        public IActionResult Login(AuthenticationModel model)
        {
            ...
            var user = userDao.GetUser(model.Email);
